...
public partial class UIChat : MonoBehaviour
{
    ...
    public bool itemLink;

    void Start()
    {
        messageInput.onValidateInput += delegate (string input, int charIndex, char addedChar) { return MyValidate(addedChar); };
    }
    ...
    public void AddMessage(ChatMessage message)
    {
        ...
        Text chatMessage = go.GetComponent<Text>();
        chatMessage.text = message.Construct();

        UIChatEntry chatEntry = go.GetComponent<UIChatEntry>();
        ...
        if (stop > -1)
        {
            ...
            {
                chatMessage.text = chatMessage.text.Replace(output, data.GetTitle());
                chatEntry.tooltip.enabled = true;
            }
        }

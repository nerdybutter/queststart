...
[System.Serializable]
public class SkillLevelingInfo
{
    public ScriptableSkill skill;  // The skill to level up
    public int rngChance = 40;     // 1 in `rngChance` chance to level up
    public bool useSkillLevelForRng;
}
...
public abstract partial class ScriptableSkill : ScriptableObject
{
    ...
    [Header("Required Item to Cast")]
    public ScriptableItem requiredItem;
    public int requiredItemQuantity;

    [Header("Leveling Configuration")]
    public List<SkillLevelingInfo> levelingSkills = new List<SkillLevelingInfo>();
    ...
    */
    ...
    public static Dictionary<int, ScriptableSkill> All
    {
        get
        {
            ...
            if (cache == null)
            {
                // Load all ScriptableSkills in the Resources folder
                ScriptableSkill[] skills = Resources.LoadAll<ScriptableSkill>("");

                // Check for duplicates and add them to the cache

...
public partial class Database : MonoBehaviour
{
    ...
    class characters
    {
        ...
        public float respawn_x { get; set; }
        public float respawn_y { get; set; }
        public float respawn_z { get; set; }
    ...
    class character_vault
    {
        public string character { get; set; }  // Character name as the key
        public int slot { get; set; }          // Slot index
        public string name { get; set; }       // Item name (hash reference)
        public int amount { get; set; }        // Stack size of the item
        public int durability { get; set; }    // Optional durability if needed
    }
    ...
    class character_scores
    {
        [PrimaryKey]
        public string character { get; set; } // The character's name
        public int score { get; set; }       // The character's score
    }
    ...
    public void Connect()
    {
        ...
        connection.CreateTable<character_scores>();
        connection.CreateTable<character_vault>();
        connection.CreateIndex(nameof(character_vault), new[] { "character", "slot" });
        // Add respawn point fields to characters table if not already present
        if (!ColumnExists("characters", "respawn_x"))
            connection.Execute("ALTER TABLE characters ADD COLUMN respawn_x FLOAT DEFAULT 0");
        if (!ColumnExists("characters", "respawn_y"))
            connection.Execute("ALTER TABLE characters ADD COLUMN respawn_y FLOAT DEFAULT 0");
        if (!ColumnExists("characters", "respawn_z"))
            connection.Execute("ALTER TABLE characters ADD COLUMN respawn_z FLOAT DEFAULT 0");
        if (!ColumnExists("accounts", "isAdmin"))
            connection.Execute("ALTER TABLE accounts ADD COLUMN isAdmin BOOLEAN DEFAULT 0");
        if (!ColumnExists("character_inventory", "durability"))
            connection.Execute("ALTER TABLE character_inventory ADD COLUMN durability INTEGER DEFAULT 0");
        if (!ColumnExists("character_equipment", "durability"))
            connection.Execute("ALTER TABLE character_equipment ADD COLUMN durability INTEGER DEFAULT 0");
    ...
    {
        // Clear current vault data to avoid duplicates
        connection.Execute("DELETE FROM character_vault WHERE character=?", vault.name);

        // Save all slots
        for (int i = 0; i < vault.slots.Count; ++i)
        {
            ItemSlot slot = vault.slots[i];
            if (slot.amount > 0)
            {
                connection.Insert(new character_vault
                {
                    character = vault.name,
                    slot = i,
                    name = slot.item.name,
                    amount = slot.amount,
    ...
    public void LoadVault(PlayerVault vault)
    {
        // Initialize all slots
        for (int i = 0; i < vault.size; ++i)
            vault.slots.Add(new ItemSlot());

        // Query the database for vault items
        foreach (character_vault row in connection.Query<character_vault>("SELECT * FROM character_vault WHERE character=?", vault.name))
        {
            if (row.slot < vault.size && ScriptableItem.All.TryGetValue(row.name.GetStableHashCode(), out ScriptableItem itemData))
            {
                Item item = new Item(itemData);
                item.durability = row.durability;
                vault.slots[row.slot] = new ItemSlot(item, row.amount);
            }
        }
    }
    ...
    {
        SaveInventory(inventory);  // Call the internal method here
    }

    public bool IsAdminAccount(string account)
    {
        var result = connection.ExecuteScalar<bool>("SELECT isAdmin FROM accounts WHERE name=?", account);
        return result;
    }
    ...
    public void SetAdminStatus(string account, bool isAdmin)
    {
        connection.Execute("UPDATE accounts SET isAdmin=? WHERE name=?", isAdmin, account);
    }
    ...
    void LoadInventory(PlayerInventory inventory)
    {
        ...
        foreach (character_inventory row in connection.Query<character_inventory>("SELECT * FROM character_inventory WHERE character=?", inventory.name))
        {
            if (row.slot < inventory.size)
            {
                ...
                {
                    Item item = new Item(itemData);
                    item.durability = row.durability > 0 ? row.durability : 100;
...
void LoadEquipment(PlayerEquipment equipment)
    ...
    public GameObject CharacterLoad(string characterName, List<Player> prefabs, bool isPreview)
    {
        ...
        if (row != null)
        {
            ...
            if (prefab != null)
            {
                ...
                player.hair                                   = row.hair;
                player.trophies = row.trophies.Length > 0
                    ? new List<int>(Array.ConvertAll(row.trophies.Split(','), int.Parse))
                    : new List<int>();
                player.bio = row.bio;
                player.respawnPoint = new Vector3(row.respawn_x, row.respawn_y, row.respawn_z);
                // Instead of reusing 'player', rename the variable to avoid the conflict.
                Player loadedPlayer = go.GetComponent<Player>();
                PlayerTime playerTime = loadedPlayer.GetComponent<PlayerTime>();

                // Use 'loadedPlayer' instead of 'player'
                if (playerTime != null)
                {
                    playerTime.playTime = row.playTime;  // Load play time from database
                    playerTime.RpcUpdatePlayTime(playerTime.playTime);  // Sync with clients
                }
    ...
    public void CharacterSave(Player player, bool online, bool useTransaction = true)
    {
        ...
        long playTimeToSave = playerTime != null ? playerTime.playTime : 0;
        ...
        connection.InsertOrReplace(new characters{
            ...
            hair = player.hair,
            trophies = string.Join(",", player.trophies),
            bio = player.bio,
            respawn_x = player.respawnPoint.x, // Save respawn x-coordinate
            respawn_y = player.respawnPoint.y, // Save respawn y-coordinate
            respawn_z = player.respawnPoint.z,  // Save respawn z-coordinate
            playTime = playerTime.playTime  // Save the updated play time
        });
    ...
    [Server]
    public void UpdateCharacterScore(string characterName, int score)
    {
        connection.InsertOrReplace(new character_scores
        {
    ...
    [Server]
    public List<KeyValuePair<string, int>> GetHighScores(int limit = 10)
    {
        if (connection == null)
        {
            Debug.LogError("[Database] No database connection. Cannot retrieve high scores.");
            return null;
        }

        var highScores = new List<KeyValuePair<string, int>>();
        var rows = connection.Query<character_scores>(
            $"SELECT character, score FROM character_scores ORDER BY score DESC LIMIT {limit}");

        foreach (var row in rows)
        {
            highScores.Add(new KeyValuePair<string, int>(row.character, row.score));
        }

...
public partial class NetworkManagerMMO : NetworkManager
{
    ...
    public EquipmentItem[] RealHairChoices;

    [HideInInspector] public ScriptableItemAndAmount RealHairChoice;
    ...
    public override void OnStartServer()
    {
        ...
        // Log all items in the database at server startup
        Debug.Log("Logging items on server startup...");
        ItemDatabaseLogger.LogAllItems();
    ...
    Player CreateCharacter(GameObject classPrefab, string characterName, string account, string hairChoice)
    {
        ...
        {
            for (int k = 0; k < RealHairChoices.Length; k++)
            {
                if (RealHairChoices[k].name == hairChoice)
                {
                    ((PlayerEquipment)player.equipment).slotInfo[8].defaultItem.item = RealHairChoices[k];
                    ((PlayerEquipment)player.equipment).slotInfo[8].defaultItem.amount = 1;
                }
            }
            // add empty slot or default item if any
                ...
                if (!Database.singleton.CharacterExists(message.name))
                {
                    ...
                    if (Database.singleton.CharactersForAccount(account).Count < characterLimit)
                    {
                        ...
                        if (0 <= message.classIndex && message.classIndex < playerClasses.Count)
                        {
                            ...
                            //{
                            // create new character based on the prefab.
                            Player player = CreateCharacter(playerClasses[message.classIndex].gameObject, message.name, account, message.hairChoice);
